...
function sendToCatBot(message_str){
    ...
    $listen_to_me_btn.text("Aguardando o weather bot...");
	g_ajaxer(G_API_GATEWAY_URL_VOICE, params, ok_cb, fail_cb);
}

function fail_cb(error){
	console.error(error);
	newBubble("gorgeous_catbot", "Todo mundo rindo de voceh, faz de novo");
	$listen_to_me_btn
			.text("Aperte p/ falar")
			.prop("disabled", false);
}
...
// console.info(JSON.parse(response.body).dialogState, report_str);
	...
	if(response.dialogState === "Fulfilled"){ // "ReadyForFulfillment"){
		...
		var 
			campus_str = response.sessionAttributes.campus_str,
			temp_int = Number(response.sessionAttributes.temp_str);



		message_str = "Ok, voce quer saber como esta o tempo em " 
				+ campus_str
				+ ". Verificando. Um segundo.";

		newBubble("gorgeous_catbot", message_str);
		setTimeout(function(){
			newBubble("gorgeous_catbot", g_customizeResponse(campus_str, temp_int));
			$listen_to_me_btn
